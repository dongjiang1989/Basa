...
class DumpTest extends PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
      $this->files_to_test = array (dirname(__FILE__).'/spyc.yaml', dirname(__FILE__).'/failing1.yaml', dirname(__FILE__).'/indent_1.yaml', dirname(__FILE__).'/quotes.yaml');
    }
    
    public function test_loadFile()
    {
        $Spyc = new Spyc;
        $filesdj = $Spyc->loadFile(dirname(__FILE__).'/failing1.yaml');
        
        $yaml_after_dump = Spyc::YAMLLoad (dirname(__FILE__).'/failing1.yaml');
        $this->assertEquals ($filesdj, $yaml_after_dump);
    }

    public function test_clearBiggerPathValues() {
        $reflection_class = new ReflectionClass("Spyc");
        $method = $reflection_class->getMethod("clearBiggerPathValues");
        $method->setAccessible(true);
        $Spyc = new Spyc;
        $Spyc->loadFile(dirname(__FILE__).'/failing1.yaml');
        $this->assertEquals( $method->invoke($Spyc, 0), true);
        $this->assertEquals( $method->invoke($Spyc, 1), true);
    
        $property = $reflection_class->getProperty("path");
        $property->setAccessible(true);
        $property->setValue($Spyc, array(2=>'aa'));
        $this->assertEquals( $method->invoke($Spyc, 1), true);
        $this->assertEquals($property->getValue($Spyc), array());
    }

    public function test_yamlizeArray() {
        $reflection_class = new ReflectionClass("Spyc");
        $method = $reflection_class->getMethod("_yamlizeArray");
        $method->setAccessible(true);
        $Spyc = new Spyc;
        $Spyc->loadFile(dirname(__FILE__).'/failing1.yaml');
        $this->assertEquals( $method->invoke($Spyc, "", 1), false);
    }

    public function test_isLiteral() {
        $reflection_class = new ReflectionClass("Spyc");
        $method = $reflection_class->getMethod("isLiteral");
        $method->setAccessible(true);
        $Spyc = new Spyc;
        $this->assertEquals( $method->invoke($Spyc, null), true);
        $this->assertEquals ( $method->invoke($Spyc, 'aaa'), true); 
        $this->assertEquals ( $method->invoke($Spyc, "aaa : bbb"), false);
    }

    public function test_loadWithSource() {
        $reflection_class = new ReflectionClass("Spyc");
        $method = $reflection_class->getMethod("loadWithSource");
        $method->setAccessible(true);
        $Spyc = new Spyc;
        function syck_load($a) {
            return array("test");
        }
        $this->assertEquals( $method->invoke($Spyc, null), array());
        $Spyc->setting_use_syck_is_possible = True;
        $this->assertEquals ( $method->invoke($Spyc, array('aaa')), array("test")); 
        $this->assertEquals ( $method->invoke($Spyc, array("aaa : bbb")), array('test'));
    }
