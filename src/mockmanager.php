...
*
*   Filename      : mockmanager.php
*   Author        : dongjiang.dongj@alibaba-inc.com
*   Last modified : 2015-4-12 19:13
*   Description : 
======================================================*/
...
final class MockManager extends BaseFactory {
    /**
    ...
    */
    ...
    /** set object to MockManager */
    private $MockObj = Array(); // '$filename' => array('$mode', '$handle')
    ...
    /** 扩展使用的接口 */
    final public static function initialize($mock) {
        return True;
    }
    ...
    * 注册Mock类型方法
    *
    *   $MockManager->RegisterPlugin('mock', Array('aaa'));
    ...
    */
    final public static function RegisterPlugin($classname, $params=""){
        if (self::check($classname)) {
            ...
            {
                //throw new RegisterFail("$classname had been Registed in MockManager: $classname \n", RETTYPE::ERR);
                logging::warn("$classname had been Registed in MockManager: \n", array_keys(self::$Register_ids));
            } else {
    ...
    * 获得具体方法的handle
    *
    *   $MockManager->getInstanceHandle('Text', array('127.0.0.1', ...));
    ...
    final public function getInstanceHandle($mode, $params="") {
        if (in_array($mode, self::getPluginMode())) {
            if (!empty($params)) {
                ...
                $_mockname = $eobj->_mockname;
                if ( in_array($_mockname, $this->getConfigInstance()) ) {
                    logging::warn($_mockname." had been instanced !\n");
                    unset($eobj);
                    return $this->MockObj[$_mockname]['handle'];
                }
                else {
                    $this->MockObj[$_mockname] = Array("mode"=>$mode, "handle"=>$eobj);
                    return $eobj;
                }
            } else {
                $reflect = new self::$Register_ids[$mode]['classname']();
                $_mockname = $reflect->_mockname;
                if ( in_array($_mockname, $this->getConfigInstance()) ) {
                    logging::warn($_mockname." had been instanced !\n");
                    unset($reflect);
                    return $this->MockObj[$_mockname]['handle'];
                } else {
                    $this->MockObj[$_mockname] = Array("mode"=>$mode, "handle"=>$reflect);
                    return $reflect;
                }
            }
        }
    ...
    */
    private function _getAllInstanceInfo($key) {
        $_tmpArr = Array();
        foreach ($this->MockObj as $k=>$value) {
            foreach ($value as $k1=>$value) {
                if ($k1 == $key)
                    $_tmpArr[$k] = $value;
            }
    ...
    */
    final public function unRegisterAllInstance() {
        foreach ($this->MockObj as $k=>$v) {
            unset($v);
            unset($this->MockObj[$k]);
        }
        return $this->MockObj == Array();
    }

    /** 
    * private方法，获得Register Mock Mode
    * @return Array
    * @author dongjiang.dongj
    */
    ...
    /** 
    * 获得Instance Mock Object
    * @return Array()
    * @author dongjiang.dongj
    */
    final public function getConfigInstance() {
        return array_keys($this->MockObj);
    }
    ...
    */
    private static function _del($var) {
        $filter_Array = Array('MockIPlugin.php', ".", ".." , "");
    ...
    /**
    * scandir ./mock/ 已经实现的 Mode，进行注册到工厂模式中
    * @return Array. 返回所有类型
    * @author dongjiang.dongj
    */
    public static function RegisterAllPlugins() {
        $_fullpath = (realpath(dirname(__FILE__).self::MOCKPATH)=="") ? 
                     dirname(__FILE__).self::MOCKPATH :
                     realpath(dirname(__FILE__))."/".basename(self::MOCKPATH);
        logging::info("Mock Plugin Path : ".$_fullpath);
        
        $RetFileArr = array_filter(scandir($_fullpath, 1), 'self::_del');
        foreach($RetFileArr as $v) {
            logging::debug(dirname(__FILE__).self::MOCKPATH.$v);
            require_once(dirname(__FILE__).self::MOCKPATH.$v);
            $classname = explode('.', $v);
