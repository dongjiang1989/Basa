...
*
*   Filename      : dictmanager.php
*   Author        : dongjiang.dongj@alibaba-inc.com
*   Last modified : 2014-11-28 08:13
*   Description : 
======================================================*/
...
final class DictManager extends BaseFactory {
    /**
    ...
    */
    ...
    /** set object to DictManager */
    private $LogObj = Array(); // '$filename' => array('$mode', '$handle')
    ...
    * 注册Conf类型方法
    *
    *   $DictManager->RegisterPlugin('conf', Array('aaa'));
    ...
    */
    final public static function RegisterPlugin($classname, $params=""){
        if (self::check($classname)) {
            ...
            {
                logging::warn("$classname had been Registed in DictManager: \n", array_keys(self::$Register_ids));
            } else {
    ...
    * 获得具体方法的handle
    *
    *   $DictManager->getInstanceHandle('LogText', array('pv.log', '127.0.0.1', ...));
        ...
        if (in_array($mode, self::getPluginMode())) {
            if (!empty($params)) {
                ...
                if ( in_array($_basename, $this->getConfigInstance()) ) {
                    ...
                    unset($eobj);
                    return $this->LogObj[$_basename]['handle'];
                }
                else {
                    $this->LogObj[$_basename] = Array("mode"=>$mode, "handle"=>$eobj);
                    return $eobj;
                }
            } else {
                ...
                if ( in_array($_basename, $this->getConfigInstance()) ) {
                    ...
                    $this->LogObj[$_basename] = Array("mode"=>$mode, "handle"=>$reflect);
                    return $reflect;
                }
            }
        }
    ...
    */
    private function _getAllInstanceInfo($key) {
        $_tmpArr = Array();
        foreach ($this->LogObj as $k=>$value) {
            foreach ($value as $k1=>$value) {
                if ($k1 == $key)
                    $_tmpArr[$k] = $value;
            }
    ...
    */
    final public function unRegisterAllInstance() {
        foreach ($this->LogObj as $k=>$v) {
            unset($v);
            unset($this->LogObj[$k]);
        }
        return $this->LogObj == Array();
    }

    /** 
    * private方法，获得Register Log Mode
    * @return Array
    * @author dongjiang.dongj
    */
    ...
    /** 
    * 获得Instance Log Object
    * @return Array()
    * @author dongjiang.dongj
    */
    final public function getConfigInstance() {
        return array_keys($this->LogObj);
    }
    ...
    */
    private static function _del($var) {
        $filter_Array = Array('DictIPlugin.php', ".", ".." , "", "tools");
    ...
    /**
    * scandir ./dict/ 已经实现的 Mode，进行注册到工厂模式中
    * @return Array. 返回所有类型
    * @author dongjiang.dongj
    */
    public static function RegisterAllPlugins() {
        $_fullpath = (realpath(dirname(__FILE__).self::DICTPATH)=="") ? 
                     dirname(__FILE__).self::DICTPATH : 
                     realpath(dirname(__FILE__))."/".basename(self::DICTPATH);
        logging::info("Log Plugin Path : ".$_fullpath);
        
        $RetFileArr = array_filter(scandir($_fullpath, 1), 'self::_del');
        foreach($RetFileArr as $v) {
            logging::debug(dirname(__FILE__).self::DICTPATH.$v);
            require_once(dirname(__FILE__).self::DICTPATH.$v);
            $classname = explode('.', $v);
