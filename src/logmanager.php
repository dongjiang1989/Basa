...
final class LogManager extends BaseFactory {
    /**
    ...
    * 注册Conf类型方法
    *
    *   $LogManager->RegisterPlugin('conf', Array('aaa'));
    ...
    */
    final public static function RegisterPlugin($classname, $params=""){
        if (self::check($classname)) {
            ...
            {
                logging::warn("$classname had been Registed in LogManager: \n", array_keys(self::$Register_ids));
            } else {
    ...
    * 获得具体方法的handle
    *
    *   $LogManager->getInstanceHandle('LogText', array('pv.log', '127.0.0.1', ...));
    ...
    */
    private static function _del($var) {
        $filter_Array = Array('LogIPlugin.php', ".", ".." , "");
    ...
    */
    public static function RegisterAllPlugins() {
        $_fullpath = (realpath(dirname(__FILE__).self::LOGPATH)=="") ? 
                     dirname(__FILE__).self::LOGPATH : 
                     realpath(dirname(__FILE__))."/".basename(self::LOGPATH);
        logging::info("Log Plugin Path : ".$_fullpath);
        
        $RetFileArr = array_filter(scandir($_fullpath, 1), 'self::_del');
        foreach($RetFileArr as $v) {
            logging::debug(dirname(__FILE__).self::LOGPATH.$v);
            require_once(dirname(__FILE__).self::LOGPATH.$v);
            $classname = explode('.', $v);
