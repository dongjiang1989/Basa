...
class PBParser
{
    ...
    var $scalar_types = array('double'=>'PBDouble', 'float'=>'PBFloat', 'int32' => 'PBInt', 'int64' => 'PBInt',
                              ...
                              'fixed32' => 'PBInt', 'fixed64' => 'PBInt', 'sfixed32' => 'PBSignedInt', 'sfixed64' => 'PBSignedInt',
     ...
     */
    public function parse($protofile, $topath)
    {
        ...
        array_pop($name);
        $name = join($name, '.');
        $this->_create_class_file($topath.'/pb_proto_' . $name . '.php');
    }
    ...
    {
        $string = "require_once('".dirname(__FILE__)."/../message/pb_message.php');\n";
    ...
    private function _create_class_body($classfile, &$string, $classname)
    {
        foreach($classfile as $field)
        {
            ...
            else if (isset($field['value']['repeated']) || $type == 'PBMessage')
            {
                $string .= '  function ' . $field['value']['name'] . '($offset)' . "\n  {\n";
    ...
    private function _create_class_constructor($classfile, &$string, $classname)
    {
        ...
        foreach($classfile as $field)
        {
            ...
            $string .= '    $this->names_type["' . $field['value']['value'] . '"] = "' . $field['value']['name'] . '"' . ";\n";
    ...
    private function _check_type($type, $array, $path)
    {
        ...
        // now try one deeper
        $_namepath = explode(".", trim($namespace, '.'));
        $namespaces  = array(trim($path . '.' . $namespace, '.'), trim($namespace, '.'), trim($_namepath[count($_namepath)-1], '.'));
        foreach ($this->m_types as $message)
        {
            if (in_array($message['name'], $namespaces))
            {
                return array($type, $message['name']);
            }
        }
        // @TODO TYPE CHECK
